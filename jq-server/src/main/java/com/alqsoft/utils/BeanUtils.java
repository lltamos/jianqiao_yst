...
public class BeanUtils {
	...
	public static AppResult uploadImage(Object o, String module) {
		...
		if (imageMap.size() != 0) {
			appResult = com.alqsoft.utils.UpLoadUtils.saveFile(imageMap, module);
	...
	public static String[] getStringArray(Class<?> oClass) {
		...
		for (Field f : fields) {
			if (f.isAnnotationPresent(com.alqsoft.annotations.Field.class)) {
				if(f.getAnnotation(com.alqsoft.annotations.Field.class).name().equals("")){
					list.add(f.getName());
				}else{
					list.add(f.getAnnotation(com.alqsoft.annotations.Field.class).name());
				}
			}
