package com.alqsoft.utils.enctypt;
...
import com.alqsoft.utils.CodeConst;

public class RequestUtil {
	
	/*******************测试使用，生产需要替换相应的值和文件*******************/
	//RSA加密使用
	static InitParamPc initParam = InitParamPc.getInitParam();
	private  final String passWord=(String) initParam.getProperties().get("password");
	private  final String pri=(String) initParam.getProperties().get("pri");
	private  final String pub=(String) initParam.getProperties().get("pub");
	private  final String rootPath=(String) initParam.getProperties().get("rsaPath");
	//签名使用
	private  final String singKey=(String) initParam.getProperties().get("singKey");//"1qa2ws3ed~!@360080000230629280";//签名key，测试环境测试的都是test，生产上一个会员对应一个key
...
//	private  final String passWord=getInitParamMap().get("password");//秘钥文件密码
//	private  final String pri=getInitParamMap().get("pri");//秘钥文件密码
//	private  final String pub=getInitParamMap().get("pub");//秘钥文件密码
//	private  final String singKey=getInitParamMap().get("singKey");//秘钥文件密码
//	private  final String rootPath=getInitParamMap().get("rootPath");//秘钥文件密码
	//参数备用方法
	/*private Map<String, String> getInitParamMap(){
		Map<String, String> map = new HashMap<>();
		InitParam initParam = InitParam.getInitParam();
		String password =(String) initParam.getProperties().get("password");
		String pri=(String) initParam.getProperties().get("pri");//秘钥文件名（该文件包含公钥和私钥）
		String pub=(String) initParam.getProperties().get("pub");//代付证书文件名
		String singKey=(String) initParam.getProperties().get("singKey");//签名key，
		String rootPath=(String) initParam.getProperties().get("rsaPath");//秘钥文件密码
		map.put("password", password);
		map.put("pri", pri);
		map.put("pub", pub);
		map.put("singKey", singKey);
		map.put("rootPath", rootPath);
		return map;
	}*/
	
	/**
	 * 请求(带证书加密) 外部商户https参考使用
	 * @throws Exception 
	 */
	public String tradeRequestSSL(Map<String, String> paramMap,String url,String encryptType,HttpServletRequest request) throws Exception{
//		String rootPath = (String) initParams.getProperties().get("rsaPath");
		//测试路径，请更改实际文件路径
		System.out.println(rootPath);
		//请求数据，http请求时将此map中的数据放入Form表单中
		Map<String,String> requestMap = enctyptData(paramMap,encryptType,request);
	 ...
	 * @throws Exception
	 */
	public String tradeRequest(Map<String, String> paramMap,String url,HttpServletRequest request) throws Exception{
		Map<String,String> requestMap = null;
		String payTool=paramMap.get("pay_tool");
		if("TRAN".equals(payTool)){//支付工具是代付（银行卡）的需做数据加密
			requestMap = enctyptData(paramMap,Contants.encryptType_DES,request);
			System.out.println("------>encrypt_data="+requestMap.get("encrypt_data"));
		}else if("XJK".equals(payTool)||"ACCT".equals(payTool)){//余额和小金库的支付工具不用加密，只做签名
			requestMap=enctyptData(paramMap,null,request);
		}else{
			requestMap=enctyptData(paramMap,null,request);//其他不用加密，只做签名
		}
		 //http请求
		WyHttpClientUtil util  = new WyHttpClientUtil();
	 ...
	 * @throws Exception
	 */
	private Map<String,String> enctyptData(Map<String, String> paramMap,String encrtyptType,HttpServletRequest request) throws Exception{
		...
		if(StringUtils.isEmpty(encrtyptType)){
			...
			if("RSA".equals(encrtyptType)){
				//测试路径，请更改实际文件路径
//				String rootPath = Thread.currentThread().getContextClassLoader().getResource(".").getPath()+"com/ydg/web/jdpay/rsa/";//获取路径
//				String rootPath = (String) initParams.getProperties().get("rsaPath");//获取路径
//				System.out.println(rootPath);
				//数据加密
...
//		data = "{\"customer_no\":\"360080002191800017\",\"out_trade_no\":\"123456d733890\",\"response_code\":\"OUT_TRADE_NO_EXIST\",\"response_datetime\":\"20150629T172719\",\"response_message\":\"外部交易号已经存在\",\"sign_data\":\"FA63D5CE42730B7AB42C35072CDD1605C204D09C6E32D7A4BEDE4575DE2C9D3F\",\"sign_type\":\"SHA-256\",\"trade_amount\":\"2\",\"trade_currency\":\"CNY\",\"trade_no\":\"20150629100041000000236659\",\"trade_status\":\"FINI\"}";
	...
	public Map<String,String> verifySingNotify(String data) throws UnsupportedEncodingException{
		...
		System.out.println("1234567890"+paramSplitStr);
